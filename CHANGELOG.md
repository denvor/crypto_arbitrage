...
1. **首页** — 新增 `GET /` 导航页，包含品牌介绍、统计栏（4 交易对 / 19,382 数据 / 8h 结算周期）、两张功能卡片链接到数据维护和回测页面，Footer 含 GitHub 项目链接
2. **深色主题** — 三页统一深色 UI（`#0f1117` 背景 + `#6366f1` 紫色主色调），CSS 变量管理配色

### 改进

3. **维护页面** — BTC/ETH/BFUSD 彩色圆点标识、记录数紫色高亮标签、状态徽章带脉冲动画（就绪/获取中/完成/失败）、Toast 通知（8 秒自动消失）
4. **回测页面** — 深色表单卡片、交易对复选框带边框卡片样式、渐变进度条、BFUSD 绿色标签、默认日期 2024-01-01 ~ 2025-01-01、默认只勾选 BTCUSDT

### Bug 修复

5. **策略方向修正** — 修正所有文档和注释中的策略方向：做空合约 + 做多现货（之前误写为做多合约 + 做空现货）
6. **品牌统一** — 全站 "taoli" 替换为 "crypto Arbitrage"

### 修改文件

| 文件 | 变更 |
|------|------|
| `templates/index.html` | **新建**：首页模板（导航栏 + Hero + 统计栏 + 功能卡片 + Footer GitHub 链接） |
| `templates/maintenance.html` | 深色主题重设计 + 彩色标识 + Toast 通知 |
| `templates/backtest.html` | 深色主题重设计 + 默认参数调整 |
| `app.py` | 首页路由替换 JSON 返回为模板渲染 + 品牌名更新 |
| `scripts/backtest.py` | 策略方向注释修正 |
| `README.md` | 策略方向修正 + 目录名更新 |
| `CLAUDE.md` | 策略方向修正 + 品牌名更新 |
